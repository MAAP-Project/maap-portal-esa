...
<c:choose>
	...
	<c:otherwise>
		...
		<div class="login-container" id="loginAdmin">
			...
			<aui:form action="<%=loginURL%>"
						...
						<c:otherwise>
							...
							<%
								Format dateFormat = FastDateFormatFactoryUtil.getDateTime(
																FastDateFormatConstants.SHORT, FastDateFormatConstants.LONG, locale,
				...
				<h5>Admin users sign-in</h5>
					...
					<%
						String urlEsa = System.getenv("ESA_NGINX_CONNECT_URL");
						String urlNasa = System.getenv("NASA_NGINX_CONNECT_URL");
						String urlGluu= System.getenv("ESA_GLUU_REGISTER_URL");
						String urlGluuPasswordReset= System.getenv("ESA_GLUU_RESET_PASSWORD_URL");
					%>
				<c:set var = "urlEsa" scope = "session" value = "<%=urlEsa%>"/>
				<c:set var = "urlNasa" scope = "session" value = "<%=urlNasa%>"/>
				<c:set var = "urlGluu" scope = "session" value = "<%=urlGluu%>"/>
				<c:set var = "urlGluu" scope = "session" value = "<%=urlGluu%>"/>
				<aui:button-row>
					<aui:button type="submit" value="Admin sign-in" id="liferaybutton"/>
				</aui:button-row>
			</aui:form>
	...
	<div class="portlet-content" style="margin-top: 10px; border: 1px solid #E7E7ED; border-radius: 0.5rem">
		<h5>NASA and ESA users sign-in</h5>
			<aui:button-row>
				<aui:button type="submit" value="Sign-in with ESA"
					onclick="window.location.href='${urlEsa}'" id="esabutton" />
			</aui:button-row>
			<aui:button-row>
				<aui:button type="submit" value="Sign-in with NASA"
					onclick="window.location.href='${urlNasa}'" id="nasabutton" />
			</aui:button-row>
	  ...
	  <a href="#" onclick="activateAdmin();">Admin</a>
	</c:otherwise>
</c:choose>

<script>
var adminLogin = document.getElementById("loginAdmin");
...
<style>
#_com_liferay_login_web_portlet_LoginPortlet_nasabutton {
	background-color: slategray;
	border-color: #0b3d91;
	width: 100%;
}

#_com_liferay_login_web_portlet_LoginPortlet_esabutton {
	background-color: #f7931e;
	border-color: #008542;
	width: 100%;
}

#_com_liferay_login_web_portlet_LoginPortlet_liferaybutton {
	background-color: grey;
	border-color: grey;
}

#_com_liferay_login_web_portlet_LoginPortlet_loginForm .input-container {
