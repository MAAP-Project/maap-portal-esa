...
// creates a draw interaction
	...
	function clearMapOverlays() {
		...
		console.log('clearing map of overlays');
		searchMap.getOverlays().forEach(function(overlay) {
	 ...
	 * @returns
	 */
	function removeMapInteractions(map,args){
	
		if (typeof args !== 'undefined' && args.length > 0) {
	
			map.getInteractions().forEach(function (interaction) {
				
				if (interaction instanceof args) {
					map.removeInteraction(interaction);
				}
			})
			
			
		}else{map.getInteractions().forEach(function (interaction) {
			map.removeInteraction(interaction);});
		}
	}
	
	/**
	 * remove all map vector layers
	 * 
	 * @returns
	 */
	function removeMapVectorLayers(map){
