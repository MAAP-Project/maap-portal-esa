	...
	<c:otherwise>
		...
		<aui:form action="<%=loginURL%>"
				...
				<c:when test='<%=SessionMessages.contains(request, "userAdded")%>'>
					...
					<div class="alert alert-success">
							...
							<c:when
								...
								<c:if test="<%=company.isStrangersVerify()%>">
									<liferay-ui:message arguments="<%=userEmailAddress%>"
										key="your-email-verification-code-has-been-sent-to-x"
						...
						<c:if
							...
							<liferay-ui:message arguments="<%=userEmailAddress%>"
								key="your-password-has-been-sent-to-x"
								translateArguments="<%=false%>" />
						</c:if>
					</div>
				</c:when>
			...
			<aui:fieldset>
				...
				<c:set var = "urlGluu" scope = "session" value = "<%=urlGluu%>"/>
			<aui:button-row>
				<aui:button type="submit" value="Sign-in with ESA" onclick="window.location.href='${urlEsa}'" id="esabutton"/>
				<aui:button type="submit" value="Sign-in with NASA" onclick="window.location.href='${urlNasa}'" id="nasabutton" />
		...
		<!-- <liferay-util:include page="/navigation.jsp"
			...
			<a href="${urlGluu}" style="font-family: 'NotesEsaReg'; color: #00338D;">+ Create account</a><a href="${urlGluuPasswordReset}" style="font-family: 'NotesEsaReg'; color: #00338D; margin-left: 30px;">+ Reset password</a>
